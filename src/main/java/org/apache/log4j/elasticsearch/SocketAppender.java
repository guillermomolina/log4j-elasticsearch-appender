...
// Contributors: Guillermo Adrián Molina <guillermoadrianmolina@hotmail.com>

/* Based on Original log4j SocketAppender but allowing layouts */
...
public class SocketAppender extends AppenderSkeleton {
   ...
   */
  public SocketAppender(Layout layout, InetAddress address, int port) {
    this.layout = layout;
   ...
   */
  public SocketAppender(Layout layout, String host, int port) {
    this.layout = layout;
    this.port = port;
  ...
  public void append(LoggingEvent event) {
    ...
    if (oos != null) {
      try {
        ...
        oos.writeObject(this.layout.format(event));
  ...
  /**
   * The SocketAppender requires a layout. Hence, this method returns
   * <code>true</code>.
   */
   ...
   * reconnect to the server when previpously open connection is droppped.
   * 
   * @author Ceki G&uuml;lc&uuml;
   * @since 0.8.4
   */
    ...
    public void run() {
      ...
      while (!interrupted) {
        try {
          ...
          LogLog.error("Connector interrupted. Leaving loop.");
          return;
        } catch (java.net.ConnectException e) {
          LogLog.error("Remote host " + address.getHostName() + " refused connection.");
        } catch (IOException e) {
          LogLog.error("Could not connect to " + address.getHostName() + ". Exception is " + e);
        }
      }
    }
  }
